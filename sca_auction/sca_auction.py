...
folder_name = os.path.join(
    cwd, "Output_folder", datetime.datetime.now().strftime("%Y-%m-%d_%H_%M_%S")
)
...
def get_page(client, url, retry=5, delay=30):
    ...
    try:
        ...
        next_page_element = html.css_first(
            "a.pagination-v2__text.pagination-v2__text--next"
        )
        ...
        if retry > 0:
            ...
            return get_page(
                client, url, retry=retry - 1, delay=delay * 2
...
def parse_detail(html, product_url):
    ...
    print(product_url)
    spec_elements = html.css(
        "div.lot-v2__panel-col.lot-v2__panel-col--third "
    ...
    for spec_element in spec_elements:
        ...
        try:
            ...
            vin_button = panel_div.find(
                "button", class_="panel-info-v2__vin"
            ).find_next_sibling("a")
            vin = vin_button["href"]
            ...
            vin_value = "NA"
        try:
            odometer_value = soup.find(
                "div", class_="panel-info-v2__desc", text=True
            ).text.strip()
        except (AttributeError, TypeError):
            odometer_value = "NA"
    sales_info = []
    spec_elements2 = html.css(
        "div.lot-v2__panel-col section.lot-v2__panel.lot-v2__panel"
    ...
    for spec_element2 in spec_elements2:
        ...
        try:
            sale_date_value = (
                soup2.find("div", class_="panel-info-v2__term", text="Sale Date")
                .find_next_sibling()
                .text.strip()
                .split(",")[0]
            )
        ...
        try:
            ...
            sales_info.append(sale_time_value)
        try:
            sale_title_state_values = (
                soup2.find("div", class_="panel-info-v2__term", text="Sale Location")
                .find_next_sibling()
                .text.strip()
            )
            ...
            sales_info.append(sale_title_state_value)
        try:
            last_updated_time_value = (
                soup2.find("div", class_="panel-info-v2__term", text="Last Updated")
                .find_next_sibling()
                .text.strip()
            )
    ...
    for spec_element1 in spec_elements1:
        ...
        try:
            vehicle_type_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Vehicle Type")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            year_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Year")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            make_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Make")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            model_detail_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Model")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            body_style_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Body Style")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            color_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Exterior Color")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            engine_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Engine")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            transmission_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Transmission")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            fuel_type_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Fuel Type")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
        ...
        try:
            cylinders_value = (
                soup1.find("div", class_="panel-info-v2__term", text="Cylinders")
                .find_next_sibling()
                .text.strip()
            )
        except (AttributeError, TypeError):
    ...
    try:
        image_thumbnail_value = (
            soup3.find("div", class_="lot-v2__panel-col")
            .find("picture", class_="lot_main_image")
            .img.get("src")
        )
    except (AttributeError, TypeError):
    ...
    spec_dict = {
        "value1": vin_value,
        "value2": odometer_value,
        "value3": vehicle_type_value,
        "value4": year_value,
        "value5": make_value,
        "value6": model_detail_value,
        "value7": body_style_value,
        "value8": color_value,
        "value9": engine_value,
        "value10": transmission_value,
        "value11": fuel_type_value,
        "value12": cylinders_value,
        "value13": sales_info[0],
        "value14": sales_info[1],
        "value15": sales_info[2],
        "value16": sales_info[3],
        "value17": image_thumbnail_value,
        "value18": image_url_value,
        "value19": sale_title_type,
        "value20": secondary_damage_value,
        "value21": repair_cost_value,
        "value22": drive_value,
        "value23": odometer_brand_value,
        "value24": est_retail_value,
        "value25": has_keys,
        "value26": lot_cond_code,
        "value27": time_zone_value,
        "value28": source_value,
        "value29": yard_number_value,
        "value30": yard_name_value,
        "value31": day_of_week_value,
        "value32": lot_number_value,
        "value33": model_group_value,
        "value34": damage_description_value,
        "value35": runs_drives_value,
        "value36": sale_status_value,
        "value37": high_bid_non_vix_sealed_vix_value,
        "value38": special_note_value,
        "value39": location_city_value,
        "value40": location_state_value,
        "value41": location_zip5_value,
        "value42": location_zip4_value,
        "value43": location_country_value,
        "value44": currency_code_value,
        "value45": create_date_time_value,
        "value46": grid_row_value,
        "value47": make_an_offer_eligible_value,
        "value48": buy_it_now_price_value,
        "value49": trim_value,
        "value50": rentals_value,
...
def parse_links(html):
    ...
    links = html.css(
        "ul.search-v2__result-lots li.result-lots__item article.result-lots__wrap a.result-lots__slide"
    )
    return {
        link.attributes["href"] for link in links
    }  # here we are using set to delete duplicate elements
...
def pagination_loop(client, max_page=20):
    ...
    url = (
        f"https://sca.auction/en/search/sale-date-{formatted_date}-to-{formatted_date}"
    )
    ...
    while True:
        ...
        all_products.extend(
            prod
        )  # Add the products from the current page to the combined list
        page_count += 1
