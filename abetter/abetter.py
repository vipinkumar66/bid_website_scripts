...
class Abetter:
        ...
        while run:
            ...
            if check_error:
                ...
                for one_div in find_div:
                    if one_div.find('a').get('href') == "/en/car-finder":
                        pass
                    else:
                        self.all_links.append(one_div.find('a').get('href'))
    ...
    def get_response(self, link):
        ...
        try:
            ...
            print(f"script_json: {script_json}")
            try:
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor() as exe_link:
        exe_link.map(abetter.get_response, abetter.all_links)
